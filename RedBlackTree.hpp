...
/*   Updated: 2023/02/18 18:55:49 by aait-oma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
struct Node
{
	...
	Node<T>*	getUncle() {
		Node<T>*	gParent = this->parent ? this->parent->parent : NULL;
		if (gParent && this->data < gParent->data)
			return gParent->right;
		else if (gParent && this->data > gParent->data)
			return gParent->left;
		return NULL;
	}
	Node<T>*    findSib() {
...
private:
	...
	/*--- insertion ---*/
	Node<T>*	rbInsertFixup(Node<T>* node, int key, Node<T>* _parent = NULL)
	{
		if (!node) {
	...
	void	fixDB(Node<T>* node)
	{
		...
		Node<T>* grandparent = sib->parent->parent;
		if (sib && sib->color == RED) {
